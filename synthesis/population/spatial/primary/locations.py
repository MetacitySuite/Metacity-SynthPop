...
def configure(context):
    context.stage("synthesis.population.spatial.primary.locations_work")
    context.stage("synthesis.population.spatial.primary.locations_edu")
...
def process(context, purpose):
    ...
    if(purpose == "work"):
        ...
        df_u = df_employed
    if(purpose == "edu"):
        _, C_kk = context.stage("synthesis.population.spatial.primary.candidates")
        _, df_employed, _, df_students = context.stage("preprocess.extract_commute_trips")
        df_u = df_students
...
"""
def execute(context):
    df_workers = context.stage("synthesis.population.spatial.primary.locations_work")
    df_students = context.stage("synthesis.population.spatial.primary.locations_edu")
    #TODO combine code from these two files
